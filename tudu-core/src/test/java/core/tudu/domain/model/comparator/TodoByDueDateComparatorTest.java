...
public class TodoByDueDateComparatorTest {
    ...
    public void compare() {
        ...
        Assert.assertEquals(8, sortedTodos.size());
        Iterator<Todo> iterator = sortedTodos.iterator();

        Todo testTodo = iterator.next();
        Assert.assertEquals("04", testTodo.getTodoId());
        testTodo = iterator.next();
        Assert.assertEquals("01", testTodo.getTodoId());
        testTodo = iterator.next();
        Assert.assertEquals("03", testTodo.getTodoId());
        testTodo = iterator.next();
        Assert.assertEquals("02", testTodo.getTodoId());
        testTodo = iterator.next();
        Assert.assertEquals("05", testTodo.getTodoId());
        testTodo = iterator.next();
        Assert.assertEquals("06", testTodo.getTodoId());
        testTodo = iterator.next();
        Assert.assertEquals("07", testTodo.getTodoId());
        testTodo = iterator.next();
        Assert.assertEquals("08", testTodo.getTodoId());
    }
}
